...
class DCGAN:
    ...
    def make_generator(self):
        ...
        self.generator.add(tf.keras.layers.Dense(2*self.imgres**2,use_bias=False, input_shape=(self.noise_dim,)))
        self.generator.add(tf.keras.layers.BatchNormalization())
        self.generator.add(tf.keras.layers.LeakyReLU())
        self.generator.add(tf.keras.layers.Reshape((int(self.imgres/4),int(self.imgres/4), 32)))
        self.generator.add(tf.keras.layers.Conv2DTranspose(32, (5,5), use_bias=False, strides=2, padding="same"))
        self.generator.add(tf.keras.layers.BatchNormalization())
        self.generator.add(tf.keras.layers.LeakyReLU())
        self.generator.add(tf.keras.layers.Conv2DTranspose(1, (5,5), strides=2, use_bias=False, activation="tanh", padding="same"))        
    ...
    def make_discriminator(self):
        ...
        self.discriminator.add(tf.keras.layers.Conv2D(32, (5,5), strides=2, input_shape=(self.imgres,self.imgres, 1), padding="same"))
        self.discriminator.add(tf.keras.layers.LeakyReLU())
        self.discriminator.add(tf.keras.layers.Dropout(0.3))
        self.discriminator.add(tf.keras.layers.Conv2D(64, (5,5), strides=2, padding="same"))
        self.discriminator.add(tf.keras.layers.LeakyReLU())
        self.discriminator.add(tf.keras.layers.Dropout(0.3))
    ...
    def discriminator_loss(self, real_estimates, fake_estimates):
        ...
        """
        ## TODO: Fill this in
        loss = logistic_loss(tf.ones_like(real_estimates), real_estimates)
        loss += logistic_loss(tf.zeros_like(fake_estimates), fake_estimates)
        return loss
